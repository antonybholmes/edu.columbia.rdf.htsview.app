...
public class MainHtsViewWindow extends ModernRibbonWindow
   ...
   * The m open panel.
   */
  private final OpenRibbonPanel mOpenPanel = new OpenRibbonPanel();

  /**
   * The m save as panel.
   */
  private final SaveAsRibbonPanel mSaveAsPanel = new SaveAsRibbonPanel();

  /**
   * The m zoom model.
   */
  private final ZoomModel mZoomModel = new ReadsZoomModel();

  /**
   * The m genomic model.
   */
  private final GenomicRegionModel mGenomicModel = new GenomicRegionModel();

  /**
   * The m y axis limit model.
   */
  private final AxisLimitsModel mYAxisLimitModel = new AxisLimitsModel();

  /**
   * The m genome model.
   */
  private final GenomeModel mGenomeModel = new GenomeModel();

  /**
   * The m resolution model.
   */
  private final ResolutionModel mResolutionModel = new ResolutionModel();

  /**
   * The m peak style model.
   */
  private final Graph2dStyleModel mPeakStyleModel = new ReadsStyleModel();

  /**
   * The m width model.
   */
  private final WidthModel mWidthModel = new WidthModel();

  /**
   * The m height model.
   */
  private final HeightModel mHeightModel = new HeightModel();

  /**
   * The m margin model.
   */
  private final MarginModel mMarginModel = new MarginModel();

  /**
   * The m track list.
   */
  private final TrackTree mTrackList = new TrackTree();
   ...
   */
  private final TitlePositionModel mTitlePositionModel = new TitlePositionModel();

  /**
   * The m annotation tree.
   */
  private final AnnotationTracksTree mAnnotationTree;
   ...
   */
  private final Map<String, Module> mModuleMap = new HashMap<>();
   ...
   */
  private class TrackEvents implements ITreeNodeEventListener {
  ...
  private class ExportCallBack implements DialogEventListener {
    ...
    public void statusChanged(DialogEvent e) {
      if (e.getStatus() == ModernDialogStatus.OK) {
        try {
          save(mFile);
        } catch (IOException | ParserConfigurationException | TransformerException | TranscoderException ex) {
          ex.printStackTrace();
        }
      } else {
        try {
  ...
  private class CloseSaveCallBack extends ExportCallBack {
    ...
    public void statusChanged(DialogEvent e) {
      if (e.getStatus() == ModernDialogStatus.OK) {
        ...
        try {
          closeSave(mPwd);
        } catch (IOException | ParseException | ParserConfigurationException | TransformerException | TranscoderException ex) {
          ex.printStackTrace();
        }
      }
    }
  ...
  private class SaveAction extends AbstractAction {
    ...
    public void actionPerformed(ActionEvent e) {
      try {
        export();
      } catch (IOException | TranscoderException | TransformerException | ParserConfigurationException e1) {
        e1.printStackTrace();
      }
    }
  }
  ...
  public final void createRibbon() {
    ...
    mFontSection.addChangeListener((ChangeEvent e) -> {
 ...
 /*
  ...
  public final void clicked(ModernClickEvent e) {
    switch (e.getMessage()) {
      ...
      break;
      case OpenRibbonPanel.FILE_SELECTED:
        try {
      ...
      break;
      case OpenRibbonPanel.DIRECTORY_SELECTED:
        try {
      ...
      case SaveAsRibbonPanel.DIRECTORY_SELECTED:
        try {
  ...
  private void heatMap()
    ...
    List<SamplePlotTrack> tracks = new ArrayList<>();
  ...
  private void readDist() {
    ...
    List<SamplePlotTrack> sampleTracks = new ArrayList<>();
  ...
  private void reads() {
    ...
    } catch (ClassNotFoundException | InstantiationException | IllegalAccessException | FontFormatException | IOException | UnsupportedLookAndFeelException e) {
      e.printStackTrace();
    }
  }
  ...
  private List<UCSCTrack> getBedGraphs() throws IOException {
    List<UCSCTrack> bedgraphs = new ArrayList<>();
