...
public class HTSTracksPanel extends TracksPanel implements ModernClickListener {
  ...
  /** The m open button. */
  private final ModernButton mOpenButton = new ModernDialogThemeButton(
      AssetService.getInstance().loadIcon(OpenTrack16VectorIcon.class, 16));

  /** The m delete button. */
  private final ModernClickWidget mDeleteButton = new ModernDialogThemeButton(
      AssetService.getInstance().loadIcon("trash_bw", 16))
          .setButtonStyle(ButtonStyle.ROUNDED);

  /** The m tracks button. */
  private final ModernClickWidget mTracksButton = new ModernDialogThemeButton(
      AssetService.getInstance().loadIcon("tracks", 16))
          .setButtonStyle(ButtonStyle.ROUNDED);

  /** The m edit button. */
  private final ModernClickWidget mEditButton = new ModernDialogThemeButton(
      AssetService.getInstance().loadIcon("edit_bw", 16))
          .setButtonStyle(ButtonStyle.ROUNDED);

  /** The m clear button. */
  private final ModernClickWidget mClearButton = new ModernDialogThemeButton(
      AssetService.getInstance().loadIcon(CrossVectorIcon.class, 16))
          .setButtonStyle(ButtonStyle.ROUNDED);

  /** The m samples button. */
  private final ModernClickWidget mSamplesButton = new ModernDialogThemeButton("Samples",
      AssetService.getInstance().loadIcon(PlusVectorIcon.class, 16))
      .setButtonStyle(ButtonStyle.ROUNDED);

  /** The m search model. */
  private final SearchModel mSearchModel = new SearchModel();

  private final GenomeModel mGenomeModel;
      ...
      if (interactive && (hasReadSupport || n > 0)) {
        ...
        if (dialog.getStatus() == ModernDialogStatus.OK) {
          if (hasReadSupport) {
            ...
            mTrackList.getRoot().addChild(new TreeNode<>(sample.getName(),
          ...
          if (peakAssembly != null) {
            ...
            for (PeakSet peaks : dialog.getShowPeaks()) {
              ...
              mTrackList.getRoot().addChild(new TreeNode<>(peaks.getName(),
